 ...
 * `036_replicate_blob_columns`, against a freshly schema-snapshotted
 * database — i.e. the exact state a real device is in immediately before
 * migration 036 ever runs. Mirrors `bootstrapDatabase`'s own loop
 ...
 * 031 itself.
 */
async function bootstrapPre036(driver: BetterSqliteDriver): Promise<void> {
  ...
  for (const migration of CORE_MIGRATIONS) {
    if (migration.name === '036_replicate_blob_columns') continue;
...
/**
 * Rebuilds migration 034's `trg_sync_capture_users_insert` in its
 ...
 * database into exactly the state migration 036 exists to repair: sync
 ...
 */
async function installPre036UsersInsertTrigger(
...
describe('core migration 036 (replicate declared-blob columns)', () => {
  it('is registered exactly once in CORE_MIGRATIONS, immediately after 030', () => {
    const names = CORE_MIGRATIONS.map((m) => m.name);
    expect(
      names.filter((n) => n === '036_replicate_blob_columns'),
    ).toHaveLength(1);
    expect(names.indexOf('036_replicate_blob_columns')).toBe(
      names.indexOf('035_create_sync_apply_conflicts') + 1,
    );
  });
  ...
  it('re-emits exactly one corrective users row (the hashed one, not the NULL-hash one), and recreates the capture trigger so a fresh INSERT captures password_hash', async () => {
    ...
    await bootstrapPre036(driver);
    await installPre036UsersInsertTrigger(driver);
    ...
    // Isolate migration 036's own effect from whatever the old trigger
    // captured on insert above.
    db.exec(`DELETE FROM sync_outbox`);

    const migration036 = CORE_MIGRATIONS.find(
      (m) => m.name === '036_replicate_blob_columns',
    )!;
    await migration036.up(driver);
    ...
    // not hex — see migration 034's jsonObjectExpr doc comment.
  ...
  it('via bootstrapDatabase, runs exactly once per database (bookkeeping table guard) even across repeated bootstraps', async () => {
    ...
    const applied = db
      .prepare(
        `SELECT COUNT(*) AS c FROM migrations WHERE name = '036_replicate_blob_columns'`,
      )
      .get() as { c: number };
