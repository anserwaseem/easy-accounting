...
- [ ] Test `Inventory.service.test.ts` builds its own inline schema instead of using `src/sql/schema.sql`. Any new migration must then be added in two places, and forgetting the second causes `no such table` failures that look unrelated to the change (this bit twice while adding `inventory_prices` / `attribute_definitions`). Load the real schema in the test setup instead.
- [ ] Sticky Name column in the inventory table. With price-list and attribute columns switched on, the table scrolls horizontally and the item code scrolls out of view, so a row's values stop being attributable to an item. Pin the Name cell (`position: sticky; left: 0`) with a matching sticky header cell, an opaque background so scrolled content does not show through, and a right border to mark the seam. Check it against the virtualised rows — the existing table uses react-virtuoso, and sticky positioning inside a virtualised container needs the offset applied to the cell, not the row wrapper.
...
- [ ] Rework the inventory table for width. With price-list and attribute columns switched on it needs constant horizontal scrolling, and 14 public attributes make that worse. Idea from the user: keep the base columns always visible and put the rest behind a per-row accordion that shows only the attributes that item actually has — most items fill a minority of the 14, so a fixed grid is mostly empty cells. Pairs with the sticky-Name-column task above; if the accordion lands, sticky may be unnecessary.
