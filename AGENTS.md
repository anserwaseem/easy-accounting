...
- **`@/` path aliases resolve at runtime only in the renderer** (webpack). The main process runs under `ts-node/register/transpile-only` with no `tsconfig-paths` hook, so a `@/...` import there survives only if TypeScript erases it — i.e. `import type`. Importing a runtime **value** (an enum such as `InvoiceType`, a function, a const) through `@/` from any module main can reach throws `Cannot find module '@/types'` at app start, while `tsc`, ESLint and Jest (which has a `moduleNameMapper`) all stay green. Use a relative import for those; `src/lib/` is shared by both processes, so its value imports must always be relative.
...
- **A purchase invoice is not always a purchase from a supplier.** It is also how a sale return is booked when an agent collects 1..N items from M customers and sends them back as one consignment. That is why purchase invoices carry `biltyNumber` and `cartons` (entry form, details, list and print) — they are consignment fields, not sale-only ones, and are left empty on a direct purchase. Note the party on such an invoice is the single vendor/agent account; the purchase form has no multi-account section view, so the M originating customers are not represented per line.
...
- **Cloud backup bucket** is `easy-accounting-backups`, created by `supabase/setup.sql`. Client never `createBucket`.
