...
- [ ] Copy attributes from one item to another. Items in a family differ by only one or two attributes (e.g. `S-23-S` matches `S-23-D` except its binding is velvet), so filling a new item means retyping ~10 identical values. Add a "Copy attributes from…" action in the per-item attribute editor: pick a source item, prefill every field from it, then edit the one or two that differ before saving. Prefilling rather than writing directly keeps the destructive case visible. Worth extending to multi-select in the inventory table (apply one source to N selected items) once the single-item case is proven.
